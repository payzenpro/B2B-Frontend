...
import React, { useState, useEffect } from "react";
import {Carousel,Row,Col,Card, Tag,Button,message,Spin,Modal,Select,Typography,Dropdown,Space,Tabs,Form,Input,  InputNumber } from "antd";
import {ShoppingCartOutlined,MailOutlined,LockOutlined,UserOutlined,ShopOutlined,PhoneOutlined,DownOutlined,MenuOutlined,} from "@ant-design/icons";
import { useNavigate } from "react-router-dom";
...
export default function Home() {
  ...
  const handleQuickAddToCart = (product) => {
    const size = product.sizes?.[0]?.label || product.sizes?.[0] || "M";
...
{loadingProducts ? (
  ...
  <Row gutter={[16, 16]}>
    {products.map((prod, index) => {
      const imgSrc = getProductImage(prod);
      const price =
        typeof prod.priceInfo?.sellingPrice === "number"
      ...
      const stock = typeof prod.stock === "number" ? prod.stock : 0;
      const vendorName =
        prod.vendor?.name || prod.vendorName || prod.brand || "Unknown";

      return (
        <Col key={prod._id || prod.id || index} xs={24} sm={12} md={8} lg={6}>
          <Card
            hoverable
            style={{ height: "100%" }}
            cover={
            ...
            <Meta
              ...
              description={
                <div>
                  ...
                  {prod.priceInfo?.mrp &&
                    prod.priceInfo.sellingPrice < prod.priceInfo.mrp && (
                      <div
                        style={{
                          fontSize: 12,
                          color: "#999",
                      ...
                      >
                        ₹{prod.priceInfo.mrp.toLocaleString()}
                      </div>
                    )}
                  <div
